...
//back wing
draw_sprite_ext(spr_dragonwings,wingdex,bodyx + xconst,bodyy,-bodyimage_xscale*4/5*wingscale,wingscale,0,COLOR,1)
...
for(i = TAILS-1; i >= 0; i -= 1)
{
    ...
    draw_sprite_ext(spr_dragonneck,1,tailx[i] + xconst,taily[i],0.9,1.5-i/12,taila[i],COLOR,1)
}
...
for(i = NECKS-1; i >= 0; i -= 1)
{
   ...
   // necka[i] = bodya
    ...
    draw_sprite_ext(spr_dragonneck,1,neckx[i] + xconst,necky[i],0.9,0.4+i/NECKS,necka[i],COLOR,1)
}

draw_sprite_ext(spr_dragonbody,1,bodyx + xconst,bodyy,1,bodyimage_xscale,bodya,COLOR,1)
...
draw_sprite_ext(spr_dragonleg,1,legx + xconst,legy,1,bodyimage_xscale,lega,COLOR,1)

//front wing
draw_sprite_ext(spr_dragonwings,wingdex,bodyx + xconst,bodyy,bodyimage_xscale*wingscale,wingscale,0,COLOR,1)

with(obj_fire)
    {draw_sprite_ext(sprite_index,image_index,x,y,image_xscale,image_yscale,direction,image_blend,image_alpha)}

draw_sprite_ext(spr_dragonhead,headindex,headx + xconst,heady,1,-image_xscale,heada,COLOR,1)
